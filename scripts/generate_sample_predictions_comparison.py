...
# RepGhostUNet
print("Loading RepGhostUNet...")
repghost_model = RepGhostUNet(n_channels=3, n_classes=1, base_ch=32, use_se=False, bilinear=False).to(device)
repghost_2017_path = os.path.join(project_root, "weights/best_repghost_unet_isic2017.pth")

# Load weights if available
models_loaded = True
for model_name, model, path in [
    ("RepGhostUNet ISIC2017", repghost_model, repghost_2017_path),
]:
    if os.path.exists(path):
        try:
...
def visualize_predictions(model, loader, dataset_name, save_prefix):
    ...
    plt.suptitle(f'RepGhostUNet Predictions vs Ground Truth - {dataset_name}', 
