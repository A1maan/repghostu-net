...
model = ESEUNet(n_channels=3, n_classes=1, base_ch=32, use_se=False, bilinear=False).to(device)
...
if __name__ == "__main__":
    ...
    best_loss = float("inf")
    best_model_path = os.path.join(weights_dir, "best_repghost_unet_isic2017.pth")
    ...
    final_model_path = os.path.join(weights_dir, "repghost_unet_isic2017.pth")
    ...
    plt.savefig(os.path.join(plots_dir, 'repghost_unet_predictions_grid_isic2017.png'))
    plt.show()
    ...
    plt.legend()
    plt.tight_layout()
    plt.savefig(os.path.join(plots_dir, 'repghost_unet_loss_curve_isic2017.png'))
    plt.show()
